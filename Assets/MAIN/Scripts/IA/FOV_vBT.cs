...
public class FOV_vBT : MonoBehaviour
{
    [Header("FOV Values")]
    ...
    public LayerMask obstacleMask, ObjToCheckMask, DonutMask;
    [Header("Other Values")]
    ...
    public List<Transform> donutList = new List<Transform>();
    ...
    {//draw field of view
        ...
        {
            if ((Vector2)agent.velocity.normalized == Vector2.zero || transform.GetComponent<GuardIAController_v2>().isTurning())
            {
                //direction_tmp = Vector2.up;
                direction_tmp = transform.GetComponent<GuardIAController_v2>().stationaryDirection;
    ...
    void FindVisiblePlayer()
    {
        ...
        for (int x = 0; x < playerInRadius.Length; x++)
        {
            ...
            if (((1 << player.gameObject.layer) & DonutMask) != 0)
            {
                Vector2 dirPlayer = new Vector2(player.position.x - transform.position.x, player.position.y - transform.position.y);
                if (Vector2.Angle(dirPlayer, direction_tmp.normalized) < (viewAngle / 2))
                {
